...
def ErrorPrinter(
    ...
    **kw: Any,
    ...
    if (
        ...
        and float(stats["error_num_total"]) / float(stats["line_num"]) * 100.0
        > max_errors
    ):
...
class TooFewFieldsError(StreamError):
    ...
    def __str__(self) -> str:
        return (
            "%(description)s, got %(fields)s but needed 6: %(line_num)s: %(line)s"
            % self.kw
        )
...
class DifferingTimestampsError(StreamError):
    ...
    def __str__(self) -> str:
        return (
            "%(description)s for %(timestamp)s: %(line_num)s: %(line)s, parts: %(parts)s"
            % self.kw
        )
...
class TooManyErrorsError(StreamError):
    ...
    def __str__(self) -> str:
        ...
        if "error_num" in res:
            res["error_lines"] = "\n" + "\n".join(
                f"  {error}: {num}" for error, num in res["error_num"].iteritems()
            )
        return (
            "%(description)s: %(error_num_total)s errors in %(line_num)s lines:%(error_lines)s"
            % res
        )
...
def normalize(
    ...
    **kw: Any,
    ...
    if not uscg:
        report_error(
            "Need to make a faster version that does not worry about the extra args and stations dict"
        )
        assert False
    ...
    for idx, origline in enumerate(nmea):
        try:
            ...
            if validate_checksum and not checksum.isChecksumValid(line):
                ...
                report_error(
                    InvalidChecksumError(line_num=stats["line_num"], line=line.strip())
                )
            ...
            timestamp = fields[
                -1
            ].strip()  # Seconds since Epoch UTC.  Always the last field
            ...
            if station is None and tagblock_station is None:
                report_error(
                    NoStationFoundError(line_num=stats["line_num"], line=line.strip())
                )
                continue
            ...
            if totNumSentences == sentenceNum:
                ...
                if ts1 == 0:
                    ...
                    if len(fields[7:-1]) == 0:
                        ...
                        out_str = (
                            checksumed_str
                            + checksum.checksumStr(checksumed_str)
                            + ","
                            + ",".join(fields[7:-1])
                        )
                else:
                ...
                if not checksum.isChecksumValid(out_str):
                    report_error(
                        InvalidChecksumInConstructedError(
                            line_num=stats["line_num"], line=line.strip()
                        )
                    )
...
def decode(
    ...
    **kw: Any,
    ...
    for tagblock, line, origline in normalize(
        nmea=nmea, errorcb=errorcb, stats=stats, **kw
    ):
        try:
