...
def TimeUtc(fields: dict[str, Any]) -> None:
    ...
    when = datetime.time(fields["hours"], fields["minutes"], int(seconds), microseconds)
    fields["when"] = when


ABK_RE_STR = (
    NMEA_HEADER_RE_STR + r"(?P<sentence>ABK),"
    r"(?P<mmsi>\d+)?,"
    r"(?P<chan>[AB])?,"
    r"(?P<message_id>\d+)?,"
    r"(?P<seq_num>\d+)?,"
    r"(?P<ack_type>\d+)" + NMEA_CHECKSUM_RE_STR
)
...
ALR_RE_STR = (
    NMEA_HEADER_RE_STR + r"(?P<sentence>ALR),"
    r"(?P<time_utc>(?P<hours>\d\d)(?P<minutes>\d\d)(?P<seconds>\d\d\.\d*))?,"
    r"(?P<id>\d+)?,"
    r"(?P<condition>[AV]),"
    r"(?P<ack_state>[AV]),"
    r"(?P<text>[^*]*)" + NMEA_CHECKSUM_RE_STR
)
...
def HandleAlr(line: str) -> dict[str, Any] | None:
    ...
    when = datetime.time(
        int(fields["hours"]), int(fields["minutes"]), int(seconds), microseconds
    )
...
BBM_RE_STR = (
    NMEA_HEADER_RE_STR + r"(?P<sentence>BBM),"
    r"(?P<sen_tot>\d),"
    r"(?P<sen_num>\d),"
    r"(?P<seq_num>\d),"
    r"(?P<chan>\d),"
    r"(?P<message_id>\d),"
    r"(?P<body>[^,*]*),"
    r"(?P<fill_bits>\d)" + NMEA_CHECKSUM_RE_STR
)
...
def HandleBbm(line: str) -> dict[str, Any] | None:
    ...
    for field in ("sen_tot", "sen_num", "seq_num", "chan", "message_id", "fill_bits"):
        result[field] = util.MaybeToNumber(fields[field])
...
FSR_RE_STR = (
    NMEA_HEADER_RE_STR + r"(?P<sentence>FSR),"
    r"(?P<id>[^,]+)?,"
    r"(?P<time_utc>(?P<hours>\d\d)(?P<minutes>\d\d)(?P<seconds>\d\d(\.\d*)?))?,"
    r"(?P<chan>[A-Z])?,"
    r"(?P<slots_recv>\d+)?,"
    ...
    r"(?P<noise_db>[-]?\d+)?,"
    r"(?P<slots_above_noise>\d+(\.d*)?)?" + NMEA_CHECKSUM_RE_STR
)
...
GGA_RE_STR = (
    NMEA_HEADER_RE_STR + r"(?P<sentence>GGA),"
    r"(?P<time_utc>(?P<hours>\d\d)(?P<minutes>\d\d)(?P<seconds>\d\d\.\d*))?,"
    ...
    r"(?P<differential_age_sec>\d+)?" + NMEA_CHECKSUM_RE_STR
)
...
TXT_RE_STR = (
    NMEA_HEADER_RE_STR + r"(?P<sentence>TXT),"
    r"(?P<sen_tot>\d+)?,"
    r"(?P<sen_num>\d+)?,"
    r"(?P<seq_num>\d+)?,"
    r"(?P<text>[^*]*)?" + NMEA_CHECKSUM_RE_STR
...
# Time in UTC.
ZDA_RE_STR = (
    NMEA_HEADER_RE_STR + r"(?P<sentence>ZDA),"
    r"(?P<time_utc>(?P<hours>\d\d)(?P<minutes>\d\d)(?P<seconds>\d\d(\.\d*)?))?,"
    r"(?P<day>\d\d)?,"
    r"(?P<month>\d\d)?,"
    r"(?P<year>\d{4})?,"
