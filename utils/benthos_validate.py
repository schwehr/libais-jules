...
MSG_VALIDATION_LOOKUP = {
    ...
    "type": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in range(1, 28)
        ),
        "good": 5,
        "bad": -1,
    },
    "repeat": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and 0 <= x <= 2**2
        ),
        "good": 4,
        "bad": -1,
    },
    ...
    "status": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in range(0, 16)
        ),
        "good": 2,
        "bad": -10,
    },
    "turn": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in range(-127, 129)
        ),
        "good": 125,
        "bad": -1000,
    },
    ...
    "lon": {
        "test": lambda x: (
            isinstance(x, float) and -180 <= x <= 180 or x == 181
    ...
    "heading": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and 0 <= x <= 359 or x == 511
        ),
        "good": 511,
        "bad": -102,
    },
    "second": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in range(0, 64)
        ),
        "good": 63,
        "bad": 64,
    },
    "maneuver": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in (1, 2)
        ),
        "good": 2,
        "bad": 3,
    },
    "raim": {"test": lambda x: isinstance(x, bool), "good": False, "bad": -2},
    "radio": {
    ...
    "shiptype": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in range(0, 100)
        ),
        "good": 87,
        "bad": str,
    },
    "to_bow": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and 0 <= x <= 2**9
        ),
        "good": 1,
        "bad": -1,
    },
    ...
    "to_port": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and 0 <= x <= 2**6
        ),
        "good": 1,
        "bad": -34,
    },
    ...
    "epfd": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in range(0, 9)
        ),
        "good": 8,
        "bad": 10,
    },
    "month": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in range(0, 13)
        ),
        "good": 12,
        "bad": "asdf",
    },
    "day": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in range(0, 32)
        ),
        "good": 1,
        "bad": ":",
    },
    "hour": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in range(0, 25)
        ),
        "good": 23,
        "bad": ",",
    },
    "minute": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and x in range(0, 61)
        ),
        "good": 54,
        "bad": "*",
    },
    "draught": {
        "test": lambda x: (
            isinstance(x, int) and not isinstance(x, bool) and 0 < x <= 2**8
        ),
        "good": 1,
        "bad": 2**8 + 1,
    },
    ...
    "dte": {"test": lambda x: isinstance(x, bool), "good": False, "bad": 8},
    # Pulled from type 18 GPSD spec
    "reserved": {"test": lambda x: x is None, "good": None, "bad": 1},
    "regional": {"test": lambda x: x is None, "good": None, "bad": -1},
    "speed": {
    ...
    "dsc": {"test": lambda x: isinstance(x, bool), "good": True, "bad": -45},
    "band": {"test": lambda x: isinstance(x, bool), "good": False, "bad": 4},
    "msg22": {"test": lambda x: isinstance(x, bool), "good": False, "bad": -2},
    "assigned": {"test": lambda x: isinstance(x, bool), "good": True, "bad": -33},
...
def validate_messages(messages, err=None):
    ...
    for msg in messages:
        ...
        if "type" not in msg or msg["type"] not in MSG_TYPE_FIELDS:
            if err is not None:
                err.write(
                    "No 'type' key in msg or type is invalid or not testable: %s" % msg
                )
            return_val = False
        ...
        # Normal field validation
            ...
            for field in MSG_TYPE_FIELDS[msg_type]:
                if not MSG_VALIDATION_LOOKUP[field]["test"](msg[field]):
                    if err is not None:
                        sys.stdout.write(
                            f"Field `{field}' failed: {json.dumps(msg) + os.linesep}"
                        )
                    return_val = False
...
class TestValidateMessages(unittest.TestCase):
    def test_all_types(self):
        for msg_type, msg_fields in MSG_TYPE_FIELDS.items():
            ...
            assert not validate_messages(
                [{"type": MSG_VALIDATION_LOOKUP["type"]["bad"]}]
            )
