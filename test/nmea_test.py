...
class LineTypeTest(unittest.TestCase):
    ...
    def testBare(self):
        ...
        self.assertEqual(nmea.LineType("$PSWDA,24.46,5.61,86.44,3.86*54"), nmea.BARE)
        self.assertEqual(nmea.LineType("$INROT,-1,A*13"), nmea.BARE)
        self.assertEqual(
            nmea.LineType("!AIVDM,1,1,,A,14Qle<001fCNvfbR75JRuBGD00Rt,0*35"), nmea.BARE
        )
        self.assertEqual(
            nmea.LineType(
        ...
        self.assertEqual(nmea.LineType("!AIVDM,1,1,,B,Dh3OwmPA5N>4,0*0F"), nmea.BARE)
    ...
    def testUscg(self):
        ...
        self.assertEqual(
            nmea.LineType(
                "$AIDLM,9,B,,,,,,,,,,,,,,,,,,,,,R*48,D05MN-BL-GBPBS1,1370786280"
            ),
            nmea.USCG,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            nmea.LineType("$AIALR,,,,,*7B,D08MN-HG-GCHBS1,1396488078"), nmea.USCG
        )
        self.assertEqual(
            nmea.LineType(
                "$SAADS,D14MN-HO-KAWBS1,112359.89,V,0,I,I*38,D14MN-HO-KAWBS1,1409484239"
            ),
            nmea.USCG,
        )
        self.assertEqual(
            nmea.LineType("$ANZDA,000258.00,03,04,2014,00,00*71,r17MHOP1,1396483379"),
            nmea.USCG,
        )

    def testTag(self):
        self.assertEqual(nmea.LineType(r"\s:station1,c:1425344187*78"), nmea.TAGB)
        self.assertEqual(nmea.LineType(r"\s:station1,c:1425344187*78\a"), nmea.TAGB)
        self.assertEqual(nmea.LineType(r"\c:1425344187,s:station1*78\c"), nmea.TAGB)
        self.assertEqual(
            nmea.LineType(
        ...
        self.assertEqual(
            nmea.LineType(
                r"\g:2-2-64856,n:154678*15\$ARVSI,r14RHAL1,,235959."
        ...
        self.assertEqual(
            nmea.LineType(
                r"\s:rORBCOMM010,q:u,c:1421625633,T:2015-01-19 "
